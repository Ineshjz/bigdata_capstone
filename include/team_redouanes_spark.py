    ...
    if with_reference:
        ...
        if Path(ref_path).exists():
            ...
            df_enriched = df_enriched.join(
                F.broadcast(ref_df.select("category", "target_revenue")),
                on="category",
                how="left",
            )
...
def run_daily(logical_date: str, *, with_reference: bool = False) -> dict:
    ...
    spark = (
        ...
        # Default is 200 partitions — overkill for ~200 rows on a laptop.
        # 4 partitions matches the local[*] core count and avoids unnecessary overhead.
        .getOrCreate()
    )
